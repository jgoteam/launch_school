...
The `Array#sort` method utilizes the `<=>` method, also known as the spaceship operator, for its elements.
With the elements being of the Length class, there is no `<=>`, though there are other custom
defined comparision methods. Thus, to fix the error, we define our own spaceship operator, comparing
the values of the lengths.
